<template>
  <div>
    <NuxtLink to="/admin/squad" class="admin-back-link">
      <font-awesome-icon :icon="['fas', 'chevron-left']" />
      All tournaments
    </NuxtLink>

    <div v-if="!tournament" class="admin-empty">Tournament not found.</div>

    <template v-else>
    <div class="members-header">
      <div>
        <p class="admin-page-kicker">{{ tournament.name }}</p>
        <h1 class="admin-page-title">Squads</h1>
        <div class="club-toggle" role="tablist" aria-label="Select team">
          <button
            v-for="c in tournament.teams"
            :key="c.key"
            type="button"
            role="tab"
            :aria-selected="c.key === selectedTeam.key"
            :class="['club-toggle-btn', c.key === selectedTeam.key && 'club-toggle-btn--active']"
            @click="selectTeam(c)"
          >
            {{ c.label }}
        ...
        <p class="club-note">{{ selectedTeam.note }}</p>
      </div>
...
// Resolve the tournament from the route slug; each of its teams maps to a
// distinct `players.team` value.
const route = useRoute();
const tournament = getTournament(route.params.tournament as string);

// Which team's roster is being managed (defaults to the first — our own).
const selectedTeam = ref<TournamentTeam>(tournament?.teams[0] ?? ({} as TournamentTeam));
...
async function save() {
  ...
  if (pendingFile.value) {
    ...
    // Namespace the image path by team so the two squads' photos never collide.
    const prefix = `${selectedTeam.value.key}-${form.squad_number || ''}-${form.name}`;
...
async function refresh() {
  players.value = await getSquad(tournament!.sport, selectedTeam.value.team);
  buildThumbs();
}

// Switch the managed team: close any open editor and reload that roster.
async function selectTeam(c: TournamentTeam) {
  if (c.key === selectedTeam.value.key) return;
  selectedTeam.value = c;
  editing.value = false;
  deleteTarget.value = null;
  loading.value = true;
  await refresh();
