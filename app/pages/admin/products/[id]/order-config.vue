    ...
    <template v-else-if="product">
      <div class="form-header">
        ...
        <div class="header-actions">
          <a :href="`/shop/${productId}/order`" target="_blank" class="preview-link">
      ...
      <template v-else>
            ...
            <div class="block-body">
              ...
              <template v-if="block.isSection">
                <div class="field-row">
                  ...
                  <input v-model="block.section" class="field-input" placeholder="e.g. Buyer Details" />
                </div>
        ...
        <!-- Totals & Filters -->
        <div class="form-card">
          <h2 class="card-title">Totals <span class="card-title-hint">(summed columns in the orders viewer)</span></h2>
          <div v-for="(t, idx) in totals" :key="t._key" class="kv-row">
            <input v-model="t.key" class="field-input" placeholder="field key (e.g. quantity)" />
            <input v-model="t.label" class="field-input" placeholder="label (e.g. Total Quantity)" />
        ...
        <div class="form-card">
          <h2 class="card-title">Filters <span class="card-title-hint">(dropdown filters in the orders viewer)</span></h2>
          <div v-for="(f, idx) in filters" :key="f._key" class="kv-row">
            <input v-model="f.key" class="field-input" placeholder="field key (e.g. size)" />
            <input v-model="f.label" class="field-input" placeholder="label (e.g. Size)" />
...
import {getProduct, updateProductOrderForm} from '~/composables/useProducts';
...
// Order-form image uploads live alongside the product, in the shared nsfm bucket
const uploadFolder = `products/${productId}/form`;
...
onMounted(async () => {
  ...
  const orderForm = product.value?.order_form as RsvpConfig | null;
  if (orderForm) {
    enabled.value = true;
    hydrate(orderForm);
  }
...
function hydrate(cfg: RsvpConfig) {
  settings.active = !!cfg.active;
  settings.start_date = isoToLocal(cfg.start_date);
  settings.close_date = isoToLocal(cfg.close_date);
  settings.closed_message = cfg.closed_message ?? '';
  settings.override_key = cfg.override_key ?? '';

  for (const item of cfg.fields ?? []) {
    if (isSection(item)) {
      blocks.push({
        _key: genKey(),
        isSection: true,
  ...
  for (const t of cfg.totals ?? []) totals.push({_key: genKey(), key: t.key, label: t.label});
  for (const f of cfg.filters ?? []) filters.push({_key: genKey(), key: f.key, label: f.label});
}
...
function removeForm() {
  if (!confirm('Remove the order form from this product entirely? Existing orders are kept but the form disappears.')) return;
  enabled.value = false;
  blocks.splice(0);
  totals.splice(0);
  filters.splice(0);
...
async function save() {
  ...
  saving.value = true;
  const {error} = await updateProductOrderForm(productId, buildConfig());
  saving.value = false;

  if (error) return flash('Failed to save order form. Please try again.', true);
  if (product.value) product.value.order_form = buildConfig();
  flash('Order form saved!');
}

async function saveNull() {
  saving.value = true;
  const {error} = await updateProductOrderForm(productId, null);
  saving.value = false;
  if (error) return flash('Failed to remove order form.', true);
  if (product.value) product.value.order_form = null;
  flash('Order form removed.');
}
</script>
